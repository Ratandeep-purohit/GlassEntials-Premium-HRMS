        ...
        if action == 'clock_in':
            ...
            if att_settings_loc and att_settings_loc.location_restriction_enabled:
                ...
                if apply_to == AttendanceSettings.APPLY_TO_SELECTED:
                    ...
                    # 'all' – existing behaviour: every employee is restricted
                    employee_is_restricted = True

                if employee_is_restricted:
...
def attendance_settings_view(request):
    ...
    att_settings, _ = AttendanceSettings.objects.get_or_create(
        ...
        defaults={
            ...
            'location_apply_to': AttendanceSettings.APPLY_TO_ALL,
    ...
    if request.method == 'POST':
        try:
            ...
            # Apply-to scope
            raw_apply_to = body.get('location_apply_to', AttendanceSettings.APPLY_TO_ALL)
            if raw_apply_to not in (AttendanceSettings.APPLY_TO_ALL, AttendanceSettings.APPLY_TO_SELECTED):
                raw_apply_to = AttendanceSettings.APPLY_TO_ALL
            att_settings.location_apply_to = raw_apply_to
            ...
            # --- Sync selected employees (M2M) ---
            # SECURITY: only accept employee IDs that belong to the admin's own org
            if raw_apply_to == AttendanceSettings.APPLY_TO_SELECTED:
                raw_emp_ids = body.get('location_restricted_employee_ids', [])
                if isinstance(raw_emp_ids, list):
                    # Validate all supplied IDs belong to this org
                    ...
                    att_settings.location_restricted_employees.set(valid_employees)
                else:
                    att_settings.location_restricted_employees.clear()
            else:
                # 'all' – clear selection (not needed, but keeps data clean)
                att_settings.location_restricted_employees.clear()
    ...
    # GET – load all active employees for the employee selector
    org_employees = Employee.objects.filter(
        organization=request.user.organization,
        is_deleted=False,
        is_active=True,
    ).select_related('designation', 'department').order_by('first_name', 'last_name')

    selected_employee_ids = list(
        att_settings.location_restricted_employees.values_list('id', flat=True)
    )
